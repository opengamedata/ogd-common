...
## @class RepositoryLocationConfig
class RepositoryLocationConfig(LocationConfig):
    ...
    def Default(cls) -> "RepositoryLocationConfig":
        return RepositoryLocationConfig(
            ...
            dataset=RepositoryLocationConfig._DEFAULT_KEY,
            other_elements={}
        )
    ...
    def _fromDict(cls, name:str, unparsed_elements:Map, key_overrides:Optional[Dict[str, str]]=None, default_override:Optional[Self]=None)-> "RepositoryLocationConfig":
        ...
        return RepositoryLocationConfig(name=name, dataset=_dataset, other_elements=unparsed_elements)

    # *** PUBLIC STATICS ***

    @staticmethod
    def FromString(name:str, key:str) -> "RepositoryLocationConfig":
        return RepositoryLocationConfig(name=name, dataset=key)
    ...
    def _toDatasetKey(dataset:Optional[DatasetKey | str], fallbacks:Map) -> DatasetKey:
        ...
        if isinstance(dataset, DatasetKey):
            ...
            ret_val = RepositoryLocationConfig._parseDatasetKey(unparsed_elements=fallbacks)
        return ret_val
