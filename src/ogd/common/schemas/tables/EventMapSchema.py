...
class EventMapSchema(TableSchema):
    ...
    def _subparseDict(cls, name:str, raw_map:Dict[str, ColumnMapElement], column_schemas:List[ColumnSchema], logger:Optional[logging.Logger]=None) -> "TableSchema":
        ...
        return EventMapSchema(name=name, column_map=_column_map, columns=column_schemas, other_elements=_leftovers)
    ...
    def RowToEvent(self, row:Tuple, concatenator:str = '.', fallbacks:Map={}) -> Event:
        ...
        if not isinstance(tstamp, datetime):
            if "timestamp" not in EventMapSchema._conversion_warnings:
            ...
            EventMapSchema._conversion_warnings["timestamp"] += 1
        ...
        if not isinstance(esrc, EventSource):
            if "esrc" not in EventMapSchema._conversion_warnings:
            ...
            EventMapSchema._conversion_warnings["esrc"] += 1
        ...
        if not isinstance(app_ver, str):
            if "app_ver" not in EventMapSchema._conversion_warnings:
            ...
            EventMapSchema._conversion_warnings["app_ver"] += 1
            app_ver = str(app_ver)
        ...
        if not isinstance(log_ver, str):
            if "log_ver" not in EventMapSchema._conversion_warnings:
            ...
            EventMapSchema._conversion_warnings["log_ver"] += 1
            log_ver = str(log_ver)
        ...
        if index is not None and not isinstance(index, int):
            if "index" not in EventMapSchema._conversion_warnings:
            ...
            EventMapSchema._conversion_warnings["index"] += 1
            index = int(index)
