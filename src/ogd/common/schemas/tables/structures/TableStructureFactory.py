...
class TableStructureFactory:
    ...
    def FromDict(name:str, all_elements:Dict[str, Any], logger:Optional[logging.Logger]=None)-> TableStructureSchema:
        ...
        match (table_type.upper()):
            case "EVENT":
                return EventTableStructureSchema.FromDict(name=name, unparsed_elements=all_elements, logger=logger)
            case "FEATURE":
                return FeatureTableStructureSchema.FromDict(name=name, unparsed_elements=all_elements, logger=logger)
            case _:
