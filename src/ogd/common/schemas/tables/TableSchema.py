...
class TableSchema(Schema):
    ...
    def _getColumns(raw_val:Any, unparsed_elements:Map, schema_name:Optional[str]=None) -> List[ColumnSchema]:
        ...
        if _column_json_list:
            ret_val = [TableSchema._getColumn(column=column) for column in _column_json_list]
        else:
    ...
    @staticmethod
    def _getColumn(column:Any) -> ColumnSchema:
        ret_val : ColumnSchema

        match column:
            case ColumnSchema():
                ret_val = column
            case dict():
                ret_val = ColumnSchema.FromDict(name=column.get("name", "UNKNOWN COLUMN NAME"), unparsed_elements=column)
            case _:
                raise TypeError(f"TableSchema raw column input was unconvertible type {type(column)}!")
